...
def process_cmdline() -> argparse.Namespace:
    ...
    sync_parser.add_argument(
        "--source-url", default="MattHomeCube", help="existing decklist, if exists"
    )
    sync_parser.add_argument(
        "--dest-url", default="LSVCube", help="list to sync to, final state"
    )
    sync_parser.add_argument("--output-dir", default="./output", help="")

    proxy_parser = sub_parsers.add_parser(
        "proxy", help="commands to help manage proxy list"
    )
    proxy_sub_parser = proxy_parser.add_subparsers(dest="proxy_command")

    proxy_import_parser = proxy_sub_parser.add_parser(
        "import", help="import existing proxylist to netdecker collection"
    )
    proxy_import_parser.add_argument(
        "--local-list", help="filepath of .txt in MTGO format"
    )
    proxy_import_parser.add_argument(
        "--remote-list", help="URL of moxfield/mtggoldfish decklist to import"
    )

    proxy_list_parser = proxy_sub_parser.add_parser(
        "list", help="list the existing proxylist/collection"
    )
    proxy_list_parser.add_argument("--output", "-o")

    proxy_free_parser = proxy_sub_parser.add_parser(
        "free", help="make proxies in supplied list available for use"
    )
    proxy_free_parser.add_argument(
        "--local-list", help="filepath of .txt in MTGO format"
    )
    proxy_free_parser.add_argument(
        "--remote-list", help="URL of moxfield/mtggoldfish decklist to import"
...
def main() -> None:
    ...
    proxy_store = SQLiteStore("sqlite:///./proxy.db")
